      ...
      <main className="hall">
        <header className="hall__header">
          ...
          <p className="hall__sub">
            ...
            <a href="https://opensea.io/collection/commitcar" target="_blank" rel="noreferrer" style={{ color: 'var(--accent)' }}>
          ...
          <div className="hall__filters">
            {TIERS.map((t) => {
              ...
              return (
                <Link key={t} href={href} className={`hall__filter ${active ? 'active' : ''}`}>
                  {t}
                </Link>
              );
            })}
            ...
            {cars.map((car) => {
              ...
              return (
                <Link key={car.id} href={`/commitcar/${car.githubUsername}`} className="hall__card">
                  ...
                  <img src={dataUri} alt={`${car.githubUsername}'s car`} style={{ width: '100%', height: 'auto', display: 'block' }} />
                  <div className="hall__card__meta">
                    <span className="hall__card__username">@{car.githubUsername}</span>
                    <span className="hall__card__rarity" style={{ color: rarity.color, background: `${rarity.color}22` }}>
